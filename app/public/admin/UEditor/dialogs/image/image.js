    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            uploader.on('uploadBeforeSend', function (file, data, header) {
                ...
                var reg = new RegExp('(^| )csrfToken=([^;]*)(;|$)')
                var arr = document.cookie.match(reg)
                var csrfToken = ''
                if (arr) {
                    csrfToken = decodeURIComponent(arr[2])
                }
                header['x-csrf-token'] = csrfToken;
                header['Authorization'] = 'Bearer ' + localStorage.getItem('votesys-token');
                header['uid'] = localStorage.getItem('votesys-userid')
            });
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                try {
                    ...
                    // if (json.url && /pictures/.test(json.url)) {
                    //     json.url = signatureUrl(json.url.substr(json.url.indexOf('pictures')));
                    // }
